 ...
 * - Delete stocks
 * 
 * Think of DAO like a librarian who manages books (data) in a library (database)
 */
public class PortfolioDAO {
    ...
    public void savePortfolioItem(PortfolioItem item) throws SQLException {
        ...
        // ? are placeholders - we'll fill them in later (prevents SQL injection attacks)
        String sql = "INSERT INTO portfolio_items (symbol, name, quantity, purchase_price, current_price) " +
                     "VALUES (?, ?, ?, ?, ?)";
        ...
        pstmt.setDouble(5, item.getStock().getCurrentPrice());  // Fill 5th ? with current price
     ...
     * @throws SQLException if database operation fails
     * 
     * Example: List<PortfolioItem> items = dao.loadAllPortfolioItems();
     */
    public List<PortfolioItem> loadAllPortfolioItems() throws SQLException {
        ...
        while (rs.next()) {
            ...
            PortfolioItem item = new PortfolioItem(stock, quantity, purchasePrice);
     ...
     * @throws SQLException if database operation fails
     * 
     * Example: List<Transaction> history = dao.loadAllTransactions();
     */
